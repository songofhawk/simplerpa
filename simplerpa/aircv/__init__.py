...
def find_all_sift(im_source, im_search, min_match_count=4, maxcnt=0):
    ...
    while True:
        ...
        tindexes = set()
        for m in good:
            # qindexes.add(m.queryIdx)  # need to remove from kp_sch
            tindexes.add(m.trainIdx)  # need to remove from kp_img
        sorted_tindex = sorted(list(tindexes), reverse=True)

        des_src = np.delete(des_src, sorted_tindex, axis=0)
        for i in sorted_tindex:
            del kp_src[i]
        pass
